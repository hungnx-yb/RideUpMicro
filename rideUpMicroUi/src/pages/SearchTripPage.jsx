...
function SearchTripPage() {
  ...
  const openApiErrorModal = (error, fallbackMessage) => {
    ...
    setErrorModalMessage(resolvedMessage);
  ...
  const fetchProvinces = async () => {
    try {
      ...
      openApiErrorModal(error, "Không tải được danh sách tỉnh/thành");
    } finally {
  ...
  useEffect(() => {
    const fetchStartWards = async () => {
      ...
      try {
        ...
        openApiErrorModal(error, "Không tải được phường/xã điểm đi");
      } finally {
  ...
  useEffect(() => {
    const fetchEndWards = async () => {
      ...
      try {
        ...
        openApiErrorModal(error, "Không tải được phường/xã điểm đến");
      } finally {
  ...
  const totalTrips = mappedTrips.length;
  ...
  const fetchTrips = async ({ startWardId = "", endWardId = "", date, showError = true }) => {
    try {
      ...
      setTrips([]);
      if (showError) {
        openApiErrorModal(error, "Không tìm được chuyến xe phù hợp");
      }
    } finally {
  ...
  const handleConfirmBooking = async ({ paymentMethod, pickupStop, dropoffStop, pickupLocation, dropoffLocation }) => {
    ...
    try {
      ...
      openApiErrorModal(error, "Không thể tạo booking. Vui lòng thử lại.");
    } finally {
      setIsBooking(false);
    }
  };
  ...
  const shouldShowPayNow = successPaymentMethod === "VNPAY" && Boolean(successBookingId);

  const handlePayNow = async () => {
    if (!successBookingId || isRedirectingPayment) {
      return;
    }
    ...
    try {
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        try {
          const url = await getPaymentUrlByBookingId(successBookingId);
          if (url) {
            paymentUrl = url;
            break;
          }
      ...
      if (!paymentUrl) {
        const message = "Chưa lấy được link thanh toán. Vui lòng thử lại sau vài giây.";
      ...
      openApiErrorModal(error, "Không lấy được link thanh toán VNPay. Vui lòng thử lại.");
      setIsRedirectingPayment(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_100%_-20%,#dcfce7_0%,#f8fafc_38%,#f1f5f9_100%)]">
      ...
      <main className="pb-9">
        <section className="relative overflow-hidden border-b border-slate-200/70 bg-slate-900 text-white">
          <img
            src={superCarBanner}
            ...
            loading="lazy"
          />
          <div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-slate-950/75 via-slate-900/70 to-emerald-900/65" />
          <div className="pointer-events-none absolute -left-20 -top-16 h-56 w-56 rounded-full bg-emerald-300/20 blur-3xl" />
          <div className="pointer-events-none absolute -bottom-20 right-0 h-64 w-64 rounded-full bg-lime-300/20 blur-3xl" />

          <div className="relative mx-auto w-full max-w-6xl px-4 pb-6 pt-6 sm:px-6 lg:px-8 lg:pb-8">
            <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-2xl">
                <p className="mb-2 inline-flex items-center gap-2 rounded-full border border-white/30 bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-emerald-100">
                  <FaBolt className="text-[11px]" />
                  Chạm để đặt xe ngay, xác nhận trong vài giây
                </p>
                <h1 className="text-2xl font-extrabold leading-tight sm:text-3xl">Đặt chuyến theo phong cách ứng dụng gọi xe</h1>
                <p className="mt-1.5 text-sm text-emerald-50/90">
                  Xem nhanh giá, giờ đi, tài xế và đặt chỗ trực tiếp nhanh như một cơn gió.
                </p>
              </div>

              <div className="grid grid-cols-3 gap-2 text-xs sm:text-sm">
                <div className="rounded-xl border border-white/25 bg-white/10 px-3 py-2 backdrop-blur-sm">
                  <p className="text-emerald-50/80">Chuyến</p>
                  <p className="text-lg font-extrabold">{totalTrips}</p>
                </div>
                <div className="rounded-xl border border-white/25 bg-white/10 px-3 py-2 backdrop-blur-sm">
                  <p className="text-emerald-50/80">Chỗ trống</p>
                  <p className="text-lg font-extrabold">{totalAvailableSeats}</p>
                </div>
                <div className="rounded-xl border border-white/25 bg-white/10 px-3 py-2 backdrop-blur-sm">
                  <p className="text-emerald-50/80">Điểm đến</p>
          ...
          <aside className="space-y-3.5">
            <article className="rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-lime-50 p-3.5 shadow-sm">
              <h3 className="flex items-center gap-2 text-sm font-extrabold uppercase tracking-wide text-emerald-800">
                <FaCalendarCheck />
                Chọn giờ đi lý tưởng
              </h3>
              <div className="mt-2.5 space-y-2 text-sm text-emerald-900/90">
                <p className="rounded-lg border border-emerald-100 bg-white/80 px-2.5 py-1.5">
                  <span className="font-semibold">06:00 - 09:00:</span> Hợp với lịch đi làm.
                </p>
                <p className="rounded-lg border border-emerald-100 bg-white/80 px-2.5 py-1.5">
                  <span className="font-semibold">09:00 - 13:00:</span> Để chọn được giá tốt.
                </p>
                <p className="rounded-lg border border-emerald-100 bg-white/80 px-2.5 py-1.5">
                  <span className="font-semibold">13:00 - 18:00:</span> Nên đặt sớm để tránh hết chỗ.
                </p>
              </div>
            </article>
            ...
            <article className="rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm">
              <h3 className="flex items-center gap-2 text-sm font-extrabold uppercase tracking-wide text-slate-700">
                ...
                Meo dat chuyen nhanh
              </h3>
              <div className="mt-2.5 space-y-2 text-sm text-slate-600">
                <p className="rounded-lg bg-slate-50 px-2.5 py-1.5">Lọc theo phường/xã gần điểm đón để ra kết quả chính xác hơn.</p>
                <p className="rounded-lg bg-slate-50 px-2.5 py-1.5">Ưu tiên chuyến có nhiều chỗ nếu bạn cần linh hoạt thay đổi lịch.</p>
                <p className="rounded-lg bg-slate-50 px-2.5 py-1.5">Kiểm tra ghi chú trên xe trước khi xác nhận đặt chỗ.</p>
              </div>
            </article>
            ...
            <article className="rounded-2xl border border-slate-800 bg-slate-900 p-3.5 text-slate-100 shadow-sm">
              ...
              <p className="mt-2 text-sm text-slate-300">
                Tài xế và chuyến đi đã được RideUp xác minh, đảm bảo an toàn và tin cậy cho bạn.
              </p>
              <button
                type="button"
                className="mt-3 inline-flex items-center gap-2 rounded-lg border border-slate-600 bg-slate-800 px-2.5 py-1.5 text-sm font-semibold text-emerald-300 hover:bg-slate-700"
              >
      ...
      >
        ...
        {isRedirectingPayment ? (
          <div className="mt-2 inline-flex items-center gap-2 rounded-lg bg-blue-50 px-2.5 py-1.5 text-xs font-medium text-blue-700">
            <FaSpinner className="animate-spin" />
            Đang tải cổng thanh toán, vui lòng chờ...
          </div>
        ) : null}
        <div className="mt-4 flex justify-end gap-2">
          {shouldShowPayNow ? (
            <button
              ...
              className="rounded-lg bg-blue-600 px-3.5 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-70"
            >
              <span className="inline-flex items-center gap-2">
                {isRedirectingPayment ? <FaSpinner className="animate-spin" /> : null}
                {isRedirectingPayment ? "Đang mở cổng thanh toán..." : "Thanh toán ngay"}
              </span>
            </button>
          ) : null}
          <button
